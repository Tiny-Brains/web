...
- **The Studio runs datalogic-rs, and so does the arena**, so they agree on the language. What
  differs is **objects**: with Templating on, the Studio treats a multi-key object as a literal, and
  the arena refuses one (every object is an operation, and an unknown key fails at *evaluation*, not
  at compile), which is why the accumulator examples are arrays. An example must evaluate in the
